...
class myThread(Thread):
    ...
    def main(self):
        ...
        #         solver.add(Implies(l[i]==l[j],tours[i][1] <= tours[j][1]))
        ...
        for i in range(m):
            ...
            #effectiveWeights.append(effectiveWeight)
            for j in range(second_dimension):
                for k in range(n+1):
            ...
            sumEffectiveWeights.append(Sum(effectiveWeight))
            solver.add(sumEffectiveWeights[-1]<=l[i])
            
        #Symmetry Breaking 2
        for i in range(m):
            for j in range(i+1,m):
                check=And(sumEffectiveWeights[i]>=l[j],sumEffectiveWeights[j]>=l[i])
                solver.add(Implies(check,tours[i][1] <= tours[j][1]))
        ...
        if str(solver.check()) != 'unsat':
            ...
            #Symmetry breaking constraint, if two couriers have same max load size, then tours rows are in alphabetic order
            # if firstCicle:
            #     for i in range(m):
            #         for j in range(i + 1, m):
            #             for k in range(2, second_dimension):
            #                 #if sum(effectiveWeights[i])>=j and sum(effectiveWeights[j])>=i:
            #                     # Enforce lexicographical order of selected items for couriers i and j
            #                 cond1 = Bool(f"symmetry_condition_{i}_{j}_{k}_1")
            #                 cond2 = Bool(f"symmetry_condition_{i}_{j}_{k}_2")

            #                     # Enforce lexicographical order of selected items for couriers i and j
            #                 solver.add(cond1 == (tours[i][1] == k))
            #                 solver.add(cond2 == (tours[j][1] == k))
            #                 solver.add(Implies(And(cond1, cond2), tours[i][0] >= tours[j][0]))
            #                 firstCicle=False
